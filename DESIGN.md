...
### Neutral
- **Warn Amber** (`--warn`, `#8a5a00`): The second saturated hue, added 2026-09-16 at the user's request for the At risk badge, and used nowhere else. It breaks the original one-hue rule deliberately: a three-state badge needs a middle, and soft ink there would read as "no information" rather than "slipping". 5.9:1 as text on the ground.
...
- **Label** (`{typography.label}`): Uppercase, soft ink. Column headers and minor section captions such as "Portfolio history".
...
- **On track / Met:** `{colors.ok}` border and text.
- **At risk:** `--warn` border and text. The middle of a three-state badge, where grey would read as "no information".
- **Behind / Missed:** alarm border and alarm text, **outlined and never filled** — a solid block here would become the largest red mass on the page and compete with the figures it describes.
- **No end date / Not started / No target / No data:** strong-rule border, soft ink text, the state carried by its glyph. The board knows nothing about their standing, so neither green nor red would be honest.
- **Pace line:** under the badge, micro uppercase in soft ink — "Day 5 of 15 · 10 left". A state judged on pace is unreadable without it.
